...
describe('Pattern model test cases', () => {
  test('constructor', () => {
    let aliveCellCoors = [[1, 0], [0, 1], [-1, -1], [0, -1], [1, -1]]
    let p = new Pattern('Glider', aliveCellCoors)

    expect(p.name).toBe('Glider')
    expect(p.alives).toEqual(aliveCellCoors)
  })

  test('asScreenCoordinate with correct width and height', () => {
    let p = new Pattern('Glider', [[1, 0], [0, 1], [-1, -1], [0, -1], [1, -1]])

    let r = p.asScreenCoordinate(11, 33)

    expect(r).toEqual([[6, 16], [5, 15], [4, 17], [5, 17], [6, 17]])
  })

  test('asScreenCoordinate with empty pattern', () => {
    let p = new Pattern('Clear', [])
  ...
  test('asScreenCoordinate with not enough width and height', () => {
    let p = new Pattern('Clear', [[10, 5], [-8, -10]])

    expect(() => p.asScreenCoordinate(20, 20)).toThrow(ValueError)
  })
})
